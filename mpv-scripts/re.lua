utils = require "mp.utils"
msg = require "mp.msg"

subs_for = {}

function main()
    local accmd = mp.get_opt("re-anitomy-cli-cmd")
    if not accmd then
        msg.fatal("re-anitomy-cli-cmd not passed!")
        return
    end
    local args = {
        "/bin/sh",
        "-c",
        [[
{
 printf '%s\0' "$@"
 printf '\0'
 find -type f -regextype posix-extended -iregex '.*\.(ass|srt)' -print0
} | ]] .. accmd .. " -z attach",
        "_", -- this will be $0
    }
    for i = 0, mp.get_property_number("playlist/count")-1 do
        table.insert(args, mp.get_property("playlist/" .. i .. "/filename"))
    end
    local resp = utils.subprocess{args=args}
    if resp.error then
        msg.fatal("mp.utils.subprocess() returned error: " .. resp.error)
        return
    elseif resp.status ~= 0 then
        msg.fatal("sh exited with non-zero code: " .. resp.status)
        return
    end
    local cur_file = nil
    for line0 in resp.stdout:gmatch(".-\0") do
        local line = line0:sub(1, -2) -- drop trailing \0
        if line == "" then
            cur_file = nil
        elseif not cur_file then
            cur_file = line
            subs_for[cur_file] = {}
        else
            table.insert(subs_for[cur_file], line)
        end
    end
    local event_found = mp.register_event("file-loaded", function(data)
        local subs = subs_for[mp.get_property("filename")]
        if subs then
            for _, sub in ipairs(subs) do
                mp.commandv("sub-add", sub)
            end
        end
    end)
    if not event_found then
        msg.fatal("file-loaded event not found!")
    end
end
main()
